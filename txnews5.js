...
const sy = init()
const signurlVal = sy.getdata('sy_signurl_txnews5')
const cookieVal = sy.getdata( 'sy_cookie_txnews5')
const videoVal = sy.getdata( 'video_txnews5')
...
if ($request && $request.method != 'OPTIONS' && $request.url.match(/user\/event\/report\?/)&&$request.body.indexOf("article_read")!= -1) {
  ...
  if (signurlVal) sy.setdata(signurlVal, 'sy_signurl_txnews5')
  if (cookieVal) sy.setdata(cookieVal,  'sy_cookie_txnews5')
...
if ($request && $request.method != 'OPTIONS' && $request.url.match(/user\/event\/report\?/)&&$request.body.indexOf("video_read")!= -1) {
  ...
  if (videoVal) sy.setdata(videoVal,  'video_txnews5')
