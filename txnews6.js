...
const sy = init()
const signurlVal = sy.getdata('sy_signurl_txnews6')
const cookieVal = sy.getdata( 'sy_cookie_txnews6')
const videoVal = sy.getdata( 'video_txnews6')
...
if ($request && $request.method != 'OPTIONS' && $request.url.match(/user\/event\/report\?/)&&$request.body.indexOf("article_read")!= -1) {
  ...
  if (signurlVal) sy.setdata(signurlVal, 'sy_signurl_txnews6')
  if (cookieVal) sy.setdata(cookieVal,  'sy_cookie_txnews6')
...
if ($request && $request.method != 'OPTIONS' && $request.url.match(/user\/event\/report\?/)&&$request.body.indexOf("video_read")!= -1) {
  ...
  if (videoVal) sy.setdata(videoVal,  'video_txnews6')
